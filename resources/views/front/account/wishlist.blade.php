...
<div class="d-sm-flex justify-content-between mt-lg-4 mb-4 pb-3 border-bottom">
    <div class="d-block d-sm-flex align-items-start text-center text-sm-start">
        <a class="d-block flex-shrink-0 mx-auto me-sm-4"
           href="{{ route('front.product', $wishlist->product->slug) }}"
           style="width: 10rem;">
            <img src="{{ asset('uploads/products/' . ($wishlist->product->images->first()->image ?? 'no-image.jpg')) }}"
    ...
    <div class="pt-2 ps-sm-3 mx-auto mx-sm-0 text-center">
        {{-- Remove button --}}
<form action="{{ route('front.account.remove', $wishlist->id) }}" method="POST">
            @csrf @method('DELETE')
